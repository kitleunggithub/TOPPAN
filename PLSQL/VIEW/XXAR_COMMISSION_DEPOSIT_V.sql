create or replace view XXAR_COMMISSION_DEPOSIT_V
AS
select 
...
(
    SELECT
        comm_rpt.*,
        nvl(TOTAL_REVENUE_HKD,0)-nvl(INTEREST,0) NET_MARGIN,
        round(decode(nvl(TOTAL_REVENUE_HKD,0),0,0,((nvl(TOTAL_REVENUE_HKD,0)-nvl(INTEREST,0))/nvl(TOTAL_REVENUE_HKD,0))*100),2) NET_MARGIN_P,
    ...
    where decode(nvl(TOTAL_REVENUE_HKD,0),0,0,((nvl(TOTAL_REVENUE_HKD,0)-nvl(INTEREST,0))/nvl(TOTAL_REVENUE_HKD,0))*100) >= fnd_com.low
    and decode(nvl(TOTAL_REVENUE_HKD,0),0,0,((nvl(TOTAL_REVENUE_HKD,0)-nvl(INTEREST,0))/nvl(TOTAL_REVENUE_HKD,0))*100) < fnd_com.high
) sales_rpt,
(
select ra.customer_trx_id,
    resource_name sales_name,
    nvl(SPLIT_PERCENTAGE,0) sales_split
from ra_customer_trx_all ra, 
    xxtm.xxbs_rep_splits xrs,
