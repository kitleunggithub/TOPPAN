...
<!--  $Header: XXAP_IBY_HSBC_FPS.xsl       1.0              2020/12/14 00:00:00 dsah    noship $   -->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="OutboundPaymentInstruction">
	...
	<!-- change concurreny request id -->
    <!-- <xsl:variable name="instrid" select="PaymentInstructionInfo/InstructionReferenceNumber"/> -->
	<xsl:variable name="instrid" select="PaymentInstructionInfo/FormatProgramRequestID"/>
...
-->		  
          <NbOfTxs>
            <xsl:value-of select="count(OutboundPayment/PaymentNumber/PaymentReferenceNumber)" />
          </NbOfTxs>
          <CtrlSum>
          ...
          <PmtInf>
            ...
            <Dbtr>
              ...
              <Id>
                <OrgId>
                  <Othr>
					<xsl:if	test="not(DocumentPayable/EmployeePaymentFlag='')">
									<xsl:choose>
										<xsl:when
											test="(DocumentPayable/EmployeePaymentFlag='Y')">  <!-- Y is paid to an employee -->
											<Id>O03</Id>
										</xsl:when>
									</xsl:choose>
									<xsl:choose>
										<xsl:when
											test="(DocumentPayable/EmployeePaymentFlag='N')"> <!-- N is paid to normal vendor -->
											<Id>O02</Id>
										</xsl:when>
									</xsl:choose>
					</xsl:if>
            ...
            <xsl:for-each select="key('contacts-by-LogicalGroupReference', PaymentNumber/LogicalGroupReference)">
              <CdtTrfTxInf>
				...
				<ChrgBr>
					<xsl:choose>
						<xsl:when test="not(Payee/SupplierDescriptiveFlexField/Attribute3='')">
							<xsl:value-of select="Payee/SupplierDescriptiveFlexField/Attribute3" />
						</xsl:when>
                ...
                <CdtrAgt>
                  <FinInstnId>
                    <ClrSysMmbId>
                      <MmbId>
					    <xsl:value-of select="concat(substring(PayeeBankAccount/BranchNumber,1,3),substring(PayeeBankAccount/BranchNumber,4,3))"/>
					  </MmbId>
                    </ClrSysMmbId>
                    <PstlAdr>
                ...
                <Cdtr>
                  <Nm>
					...
					<!-- <xsl:value-of select="substring(PayeeBankAccount/BankAccountName,1,20)"/> 20210615 if back account name is null show payee name-->
					<xsl:choose>
